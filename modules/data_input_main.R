...
dataInputServer <- function(id, shared, load_spectral_data_memo) {
  moduleServer(id, function(input, output, session) {
    ...
    dataInput <- reactive({
      ...
      smp_ID <- df[[1]]   # a plain character/numeric vector
      MIR    <- df[, 2:ncol(df)]
    ...
    observeEvent(input$predict, {
      ...
      if (shared$mlModel == "PLS") {
        ...
        if (is.null(predictions) || length(predictions) == 0) {
          ...
          return(invisible(NULL))  # <-- IMPORTANT: don't fall through to the common tail
        }
        
        # length check (defensive)
        if (length(predictions) != length(smp_ID)) {
          showNotification(sprintf("Prediction length (%d) did not match Sample ID count (%d).",
                                   length(predictions), length(smp_ID)),
                           type = "error")
          shared$predictions <- NULL
          return(invisible(NULL))
        }
        
        outputFile <- data.frame(
          "Sample ID"  = smp_ID,
    ...
    output$downloadData <- downloadHandler(
      ...
      content = function(file) {
        # Guard: no predictions yet → show nice error and cancel the download
        shiny::validate(
          shiny::need(!is.null(shared$predictions) && nrow(shared$predictions) > 0,
                      "No predictions available. Click “Make Prediction” first.")
        )
        tmpdir <- tempdir()
        pred_path <- file.path(tmpdir, "predictions.csv")
        meta_path <- file.path(tmpdir, "metadata.txt")
        
        # write files
        utils::write.csv(shared$predictions, pred_path, row.names = FALSE)
        ...
        writeLines(metadata_content, meta_path)
        
        # zip them up (no setwd games needed)
        files_to_zip <- c(pred_path, meta_path)
        tryCatch({
          zip::zipr(zipfile = file, files = files_to_zip)
        }, error = function(e) {
          # Emit a user-facing notification and abort the download cleanly
          showNotification(paste("Failed to create ZIP:", e$message), type = "error")
          stop("ZIP creation failed")  # causes Shiny to cancel the download
        })
      },
